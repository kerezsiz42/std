 ...
 * @experimental **UNSTABLE**: New API, yet to be vetted.
 *
 * Extends {@linkcode Handler} by adding adding a first `error` argument.
 *
 ...
 * @param handler Default request handler: runs for any method
 */
...
/**
 * RouteWithMethodHandlers subtype of {@linkcode Route}.
 *
 * @experimental **UNSTABLE**: New API, yet to be vetted.
 *
 * @param pattern Request URL pattern
 * @param handlers Request handlers: an object with methods as keys and handlers as values
 */
export type RouteWithMethodHandlers = {
  pattern: URLPattern;
  handler?: never;
 ...
 * @experimental **UNSTABLE**: New API, yet to be vetted.
 */
export type Route = RouteWithDefaultHandler | RouteWithMethodHandlers;
 ...
 * then returns a 405 response. Returns a generic 404 response when no route matches.
 * Catches errors thrown by a handler and returns a generic 500 error, or handles
 * the error with the provided errorHandler when available.
 *
 * @experimental **UNSTABLE**: New API, yet to be vetted.
 *
 * @example Usage
